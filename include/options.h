...
namespace chess {
  ...
  public:
    Options() = default;
    Options(int const argc, char** argv) { parse(argc, argv); }
    ...
    [[nodiscard]] bool exists(string& key) const { return options.find(key) != options.end(); }
    ...
    void set(char const* key, char const* value = "") { options[string(key)] = string(value); }

    void setInt(char const* key, int const value = 0) { options[string(key)] = to_string(value); }
